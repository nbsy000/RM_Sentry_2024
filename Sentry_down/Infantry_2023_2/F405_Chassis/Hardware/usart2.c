...
void VOFA_Send(void)
{
	...
	//需转为float型数据存储
	VOFA_justfloat[0]=(float)(JudgeReceive.self_outpost_hp);
	VOFA_justfloat[1]=(float)(JudgeReceive.self_base_hp);   
	VOFA_justfloat[2]=(float)(JudgeReceive.remainHP);
	VOFA_justfloat[3]=(float)(F105.Sendmessage.commd_keyboard);
	VOFA_justfloat[7]=(float)(JudgeReceive.remain_time);
	VOFA_justfloat[8]=(float)(F405.PC_Mode);
	VOFA_justfloat[9]=(float)(F405.nav_path_state);
