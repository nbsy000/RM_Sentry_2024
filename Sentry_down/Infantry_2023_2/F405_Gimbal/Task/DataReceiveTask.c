...
uint8_t last_commd_keyboard;
uint8_t keyboard_update_flag = 0;
void Can1Receive0(CanRxMsg rx_message1)
{
	switch (rx_message1.StdId)
	{
	case 0x094://裁判系统信息
//		memcpy(&F105.JudgeReceive_infzo,rx_message1.Data,sizeof(JudgeReceive_Info_Typedef));
//		Shoot.HeatControl.HeatUpdateFlag = 1;
	...
	//哨兵
		...
		if(F105.JudgeReceive_info.heat_update)
			Shoot.HeatControl.HeatUpdateFlag = 1;

		Robot_Disconnect.F105_DisConect=0;
		if(last_commd_keyboard != F105.JudgeReceive_info.commd_keyboard)
			keyboard_update_flag = 1;
		else
			keyboard_update_flag = 0;
		last_commd_keyboard = F105.JudgeReceive_info.commd_keyboard;
		break;
	case 0x096://大符信息
