...
void Shoot_PC_Cal()
{
	...
	if((NAV_car.Shoot_PC_State == ARMOR_STATE)||//辅瞄模式
		((NAV_car.Shoot_PC_State == NAV_STATE)&&(armor_state == ARMOR_AIMED)&&ABS(pc_yaw - Gimbal.Yaw.Gyro) < 70 && ABS(pc_pitch - (Gimbal.Pitch.Gyro)) < 60 &&(NAV_car.NAV_Aim_Mode==BOTH_NAV_AIM)&& (distance < 3.0)) )	//导航辅瞄模式 识别到
	{
