...
int16_t Random_w_generate(int w_base)
{
	...
	{
		A_rand = rand()%3000 ;  //A_rand 范围1000
...
//		if(NAV_car.NAV_State != TO_PATROL)//回巡逻区时需要小陀螺
		if(1)
		{
			if ((-3.1416f / 2) > Theta)
			{
		...
		{
				follw_speed = 0;//LIMIT_MAX_MIN(((distance - 2.0f)*1300 > 0 ? (distance - 2.0f)*1300 :0),2000,-2000)*follow_able;//临时把追击关了，有点危险——8.02
