...
void ChassisCan1Send(short *carSpeedx, short *carSpeedy, short *carSpeedw)
{
	...
	*carSpeedy = LIMIT_MAX_MIN(*carSpeedy,2000,-2000);
	...
	if(Status.ControlMode == Control_RC_Mode)//遥控器模式
	{
		*carSpeedx = LIMIT_MAX_MIN(*carSpeedx,1000,-1000);
		*carSpeedy = LIMIT_MAX_MIN(*carSpeedy,1000,-1000);
	}
...
//		F405_Send->Mag_Flag = 0;
	...
	F405_Send->PC_Mode = NAV_car.NAV_State;
	F405_Send->nav_path_state = NAV_car.NAV_Path_State;
	...
	memcpy(&tx_message.Data[6], &F405_Send->PC_Mode, 1);
	memcpy(&tx_message.Data[7], &F405_Send->nav_path_state, 1);		
	CAN_Transmit(CAN1, &tx_message);
}
...
void TX2_task(void *pvParameters)
{
	...
	while (1)
	{
		...
		IWDG_Feed();
		vTaskDelayUntil(&LastWakeTime_PC, 5);
