...
//			else
//			chassis.carSpeedy= 0; 
			chassis.carSpeedx = -(-1024+RC_Ctl.rc.ch0)*SinSP_Theta+(-1024+RC_Ctl.rc.ch1)*CosSP_Theta;
			chassis.carSpeedy = +(-1024+RC_Ctl.rc.ch1)*SinSP_Theta+(-1024+RC_Ctl.rc.ch0)*CosSP_Theta;
	}
	if(Status.ControlMode==Control_MouseKey_Mode)  
	{
...
int squarecount = 0;
int i;
void Chassis_Square_Cal(Remote rc,Key key) 
{
	if(ChassisAct_Init_Flag!=Chassis_Square_Mode)
	{
		chassis.carSpeedw = 0;
    ChassisAct_Init_Flag=Chassis_Square_Mode;
	}
	Theta=ChassisPostionAngle_TranSform(Infantry.Yaw_init)/360.0f*6.28318f;  //弧度
...
void Chassis_CurrentPid_Cal(void)
{
	switch (Status.ChassisMode)
	{
		...
		case Chassis_Square_Mode:
			Chassis_Square_Cal(RC_Ctl.rc,RC_Ctl.key);
			break;
...
#elif Robot_ID == 5
	/********************************************* 5 号车 ***********************************************************/	
		pidChassisPosition.P = -6.0f;				//  位置环					3号车
	  pidChassisPosition.I = 0.00f;					
	  pidChassisPosition.D = -9.0f;	
	  pidChassisPosition.IMax = 300.0f;
	  pidChassisPosition.OutMax = 16000.0f;
	  pidChassisPosition.DeadZone=0.0f;
		pidChassisPosition.RC_DF = 0.105f;
...
#elif Robot_ID == 44
	/********************************************* 44 号车 ***********************************************************/	
		pidChassisPosition.P = 9.0f;				//  位置环					44号车
	  pidChassisPosition.I = 0.00f;					
	  pidChassisPosition.D = 0.0f;	
	  pidChassisPosition.IMax = 300.0f;
	  pidChassisPosition.OutMax = 16000.0f;
