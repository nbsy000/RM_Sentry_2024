...
//将要输出的变量声明extern
extern Pid_Typedef PidYawAidSpeed;
extern short YawCurrent;
extern Pid_Typedef PidYawAidPos;

void VOFA_Send(void)
{
	//需转为float型数据存储
	VOFA_justfloat[0]=(float)(PidYawAidPos.ActualValue);
	VOFA_justfloat[1]=(float)(PidYawAidPos.SetPoint);
