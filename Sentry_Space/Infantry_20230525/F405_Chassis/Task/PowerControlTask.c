...
void ADC_Filter(void)
{
	...
	UCAP = ((SUM_UCAP/30.0f*825)/1024.0f)/1000.0f;
	UDCDC = ((SUM_UDCDC/30.0f*825)/1024.0f)/1000.0f;
	float temp_temperature;
	temp3v3 = ((SUM_TEMP/30.0f*825)/1024.0f)/1000.0f;
...
float ChargeCal()	//由剩余功率计算充电电流
{
    ...
    else if(AD_actual_value <= 22.0f && I_Set >= 1.0f)
    {
        ChargeCtl.SetPoint -= 100;
        ChargeCtl.SetPoint = ABS(ChargeCtl.SetPoint);
    }
    else if(AD_actual_value > 22.0f)
    {
        ChargeCtl.SetPoint = 0;
    }
