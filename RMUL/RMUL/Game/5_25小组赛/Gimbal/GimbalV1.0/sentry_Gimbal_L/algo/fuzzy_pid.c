...
//给pitch轴用的参数
#define stair 0.4
#define pstair 30 //与普通PID的参数可能值有关
#define istair 0.00002
#define dstair 0.005
//给yaw轴用的参数
#define yaw_stair 0.4
#define yaw_pstair 0.03
...
static const float fuzzyRuleKd[7][7]={
 ...
 //关键算法
static Fuzzy_Typedef fuzzy(float e,float ec)
{
     ...
     if(e>PL||e<NL)
	    ...
	    if( etemp == 0)
				...
				eRighttemp=(((double)e-etemp)/stair);
	...
	eRightIndex=(short) (eLeftIndex+1);
	  ...
	  //计算EC隶属度
	    ...
	    if( ectemp == 0)
				...
				ecRighttemp=(((double)ec-ectemp)/stair);
