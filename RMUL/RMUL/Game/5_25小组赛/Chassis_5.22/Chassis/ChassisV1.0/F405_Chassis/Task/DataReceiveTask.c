...
//          is_game_start = (JudgeReceive.game_type == 0x01 && JudgeReceive.game_progress >=0x04)?1:0;
				...
				{	
					...
					if(JudgeReceive.commd_keyboard == 'A')//左移动
							Patrol_Cnt = LIMIT_MAX_MIN(Patrol_Cnt++,2,0);
					if(JudgeReceive.commd_keyboard == 'D')//右移动
							Patrol_Cnt = LIMIT_MAX_MIN(Patrol_Cnt--,2,0);
				}
...
void Can2Receive0(CanRxMsg rx_message)
{
    ...
    else if (rx_message.StdId == 0x101)
    {
		...
		case NAV_DATA_ID:
				...
				chassis.Radar_totalX = PCReceive.now_x;
				chassis.Radar_totalY = PCReceive.now_y;
				chassis.Alpha = (PCReceive.now_w/1000.0f)*180.0f/PI - signalAngle/100.0f;
				break;
		case GIMBAL_SYNE_ID:
