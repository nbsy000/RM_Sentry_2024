...
enum POWERSTATE_Typedef PowerState = BAT;//发电状态，初始是电池供电
enum CHARGESTATE_Typedef ChargeState = ChargeOff;//充电状态，初始是不进行充电
...
void ChargeIO_Configuration(void)
{
	...
	GPIO_SetBits(GPIOA,GPIO_Pin_7);	//拉高Diode Mode   拉高才能正常工作
	/***********************放电选择使能***********************/
...
{
	float i;
	MaxChargePower = 1000 * (JudgeReceive.MaxPower-5) ; //最大充电功率比裁判系统给的小5w
	if(PowerState == BAT)   //0 使用电池放电 此时电池给电容充电，电池给底盘供电
	{
		...
		//实际：电容充电功率  SetPoint: 希望充电的功率 单位mW   INA260_2.Power  底盘实际消耗功率
	}
	else 
	{
		ChargeCtl.SetPoint = MaxChargePower;
		//现在是处于超级电容给底盘供电，电池给超级电容供电
	}
  	ActualPower = INA260_1.Power - INA260_2.Power;
	i = PID_Calc(&ChargeCtl,ActualPower)/1000; //PID 计算的到超级电容需要的充电电流
	return i;
}
/**********************************************************************************************************
*函 数 名: HalfCAPCal
...
{
	/******************充电控制*******************/
   I_Set = ChargeCal(); //计算当前超级电容需要的充电电容
	if(PowerState == BAT)  //bat
	{
		...
		else if(Chassis_Run_Flag) //前后方刹车或变相
		{
			I_Set = 0;
		}
	}
