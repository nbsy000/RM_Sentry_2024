...
/**
  ...
  * @param  Data to check,Stream length, initialized checksum
  * @retval CRC checksum
  */
...
/**
  ...
  * @param  Data to Verify,Stream length = Data + checksum(dwLength=????+?????2,??????)
...
/**
  ...
  * @param  Data to CRC and append,Stream length = Data + checksum(dwLength=????+?????2,??????)
  * @retval None //True or False (CRC Verify Result)
  */
...
/**
  ...
  */
int16_t sss; //???
uint32_t Verify_CRC8_Check_Sum(uint8_t *pchMessage, uint32_t dwLength)//Verify_CRC8_Check_Sum(tempPC,PC_RECVBUF_SIZE)
{
