...
float patrol_step_pitch = 0.06f/*0.035f */, patrol_step_yaw =0.03f;// 0.005f;                    //巡逻设定值更新
...
float K_AIM_pitch = 0.6;
float K_AIM_yaw = 0.25;
...
void Gimbal_Cruise(Gimbal_Typedef *Gimbal,uint8_t Cruise_Mode)
{
		...
		step_speed_pitch = patrol_step_pitch*sqrt(1 - pow((MotoPitch->PCsetAngle-(MotoPitch->PATROL_MAX_ANGLE+MotoPitch->PATROL_MIN_ANGLE)/2.0)*2.0/abs(MotoPitch->PATROL_MAX_ANGLE-MotoPitch->PATROL_MIN_ANGLE),2));
...
void Gimbal_Limit_Init(void)
{
    ...
    MotoPitch_L.PATROL_MIN_ANGLE =-(4762.0f-4612)/8192.0f*360.0f;
		...
		//4169  7068
    MotoYaw_L.PATROL_MAX_ANGLE = (5500.0f-4870)/8192.0f*360.0f;
    MotoYaw_L.PATROL_ZERO_POS =  0;
    MotoYaw_L.PATROL_MIN_ANGLE =-(4870.0f-4400)/8192.0f*360.0f;
}
...
/*陀螺仪反馈*/
    ...
    MotoYaw.PidSpeed.IMax = 900.0f;
	  ...
	  MotoYaw.K1 = 0.0f;
		...
		//左云台
		//左云台
    MotoPitch_L.PidPos.P = 16.0f;//7.1f;//20.0f;
    ...
    MotoPitch_L.PidSpeed.D = 1600;//4.0f;                
    MotoPitch_L.PidSpeed.IMax = 2800.0f;
