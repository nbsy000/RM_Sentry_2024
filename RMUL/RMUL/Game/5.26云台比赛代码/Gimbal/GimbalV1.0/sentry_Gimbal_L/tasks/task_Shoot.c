...
uint8_t Heat_ShootAbleFlag_0 = 0; //底盘给过来的热量控制能否打弹标志位
uint8_t Heat_ShootAbleFlag = 0;
...
uint32_t delayTick_oneShot = 30;//60;//125; //走掉一颗弹丸间隔的时间(ms)
...
void task_Shoot(void *parameter)
{
    ...
    while (1)
    {
        Shoot_ModeUpdate_Flag = (Shoot_LastMode != Sentry_State.Shoot_L_Mode); 
        Shoot_LastMode = Sentry_State.Shoot_L_Mode;                              //更新上次状态
        ...
        BodanDelay_OVER = (BodanDelay_Tick >= BodanDelay_Threshold);                //根据延时时间和门限值，判定延时是否结束
				...
				aiming();//瞄准判断
        if (Sentry_State.Shoot_L_Mode == Shoot_L_PC)					Shoot_PC_Act();
        else if (Sentry_State.Shoot_L_Mode == Shoot_L_RC)      Shoot_RC_Act();
        else if (Sentry_State.Shoot_L_Mode == Shoot_L_SLEEP)   Shoot_SLEEP_Act();
        else if (Sentry_State.Shoot_L_Mode == Shoot_L_DEBUG)   Shoot_DEBUG_Act();
...
static void Shoot_PC_Act(void)
{
    ...
    extern uint8_t CV_Shoot_ABLE;     //判定视觉方面是否能够打子弹
		...
		if(!is_game_start)
					delayTick_oneShot = 90;
    if (Heat_ShootAbleFlag&& CV_Shoot_ABLE)
    {
        shootDelayTick++;
...
{
		//热量控制的调电检测
		if(Robo_Disconnect.HeatDiscount > 500)
		{
			delayTick_oneShot = 90;
			Heat_ShootAbleFlag = 1;
		}
...
static void PID_Shoot_Init(void)
{
    ...
    BodanMotor.pid_pos.I = 0.0f;//0.0f;
    BodanMotor.pid_pos.D = 0.2f;//10.0f;
    BodanMotor.pid_pos.IMax = 00.0f;//0.0f;
