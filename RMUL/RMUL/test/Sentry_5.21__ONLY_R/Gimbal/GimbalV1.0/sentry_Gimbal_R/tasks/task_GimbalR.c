...
float patrol_step_pitch = 0.015f/*0.035f */, patrol_step_yaw =0.03f;// 0.005f;                    //巡逻设定值更新
...
void Gimbal_PC_Act(uint8_t Syne_Flag)
{
    if (Gimbal_L.ModeUpdate_Flag)//只要一个云台更新就够
    {
        ...
        MotoPitch_L.PCsetAngle = MotoPitch_L.actualAngle;
			...
			{
					ChassisYaw_Cnt = 500;//3*1000*PI;//识别到后进行3周的正弦巡航
...
/*陀螺仪反馈*/
		//主云台
    MotoPitch.PidPos.P = 18.0f;//100;//15.0f;//7.0;//7.3;//25.0f;
    ...
    MotoYaw.PidSpeed.IMax = 900.0f;
	  ...
	  MotoYaw.K1 = 0.0f;
		...
		//左云台
    MotoPitch_L.PidPos.P = 15.0f;//7.1f;//20.0f;
    MotoPitch_L.PidPos.I = 0.2;//0.32f;
    MotoPitch_L.PidPos.D = 10;//6.0f;
    MotoPitch_L.PidPos.IMax = 55;//90.0f;
    MotoPitch_L.PidPos.RC_DF = 0.5;//没有很明显的滤波效果
		MotoPitch_L.PidPos.OutMax = 125.0f;

    MotoPitch_L.PidSpeed.P = 180;//220.0f;//150.0f;
    MotoPitch_L.PidSpeed.I = 3;//1.45f;
    MotoPitch_L.PidSpeed.D = 2100;//4.0f;                
    ...
    //////////////////////////////////////////////////////////////
    MotoYaw_L.PidPos.P = 16.0f;//18.0f;
    ...
    MotoYaw_L.PidSpeed.P = 190.0f;//230.0f;
