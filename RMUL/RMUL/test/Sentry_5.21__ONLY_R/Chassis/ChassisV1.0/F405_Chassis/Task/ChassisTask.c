...
void Chassis_Patrol_Act()
{
	...
	if(((RC_Ctl.rc.ch0-1024)>300)||(JudgeReceive.commd_keyboard=='D'))//右右
		chassis.PC_State = OUTPOST;//前哨站
	if(((RC_Ctl.rc.ch0-1024)<-300)||(JudgeReceive.commd_keyboard=='A'))//右左
		chassis.PC_State = PATROL;//巡逻区
	if(((RC_Ctl.rc.ch1-1024)>300)||(JudgeReceive.commd_keyboard=='W'))//右上
		chassis.PC_State = TOPATH1;//去前哨站
	if((RC_Ctl.rc.ch1-1024)<-300||(JudgeReceive.commd_keyboard=='S'))//右上
...
//				if(chassis.NAV_State == FINISHED)//路径完成
//						chassis.PC_State = OUTPOST;//前哨站
				break;
			...
			case BACKPATH1://路径回
				...
				chassis.NavigatePathNum = 2;//去前哨战
				NAV_PATH_Act(RADAR);
...
void Chassis_Patrol_Act2(void)
{
	...
	{
		init_x = chassis.Radar_totalX;
		init_y = chassis.Radar_totalY;
		Chassis_Last_State = Chassis_Patrol;
		count = 0;
	}
	...
	offset_x = init_x - PCReceive.now_x;
	offset_y = init_y - PCReceive.now_y;
...
void Outpost_Act()
{
	...
	if(chassis.PC_State != chassis.Last_PC_State)
	{
		init_x = chassis.Radar_totalX;//通过改这个值来改变移动的中心位置
		init_y = chassis.Radar_totalY;
	}
	...
	//计算得到底盘x，y，w三向速度大小
	Position_Control(init_x,init_y,Theta);
...
void Patrol_Act()
{
	...
	if(chassis.PC_State != chassis.Last_PC_State)
	{
		init_x = chassis.Radar_totalX;//通过改这个值来改变移动的中心位置
		init_y = chassis.Radar_totalY;
		PC_setx = chassis.Radar_totalX-MAX_DISTANCE;
		PC_sety = chassis.Radar_totalY;
	}
	float Theta = (chassis.Alpha - chassis.N_Yaw_angle_init)/360.0f*2*PI;//θ角为与y方向的夹角，具体符号取值与运动分解的θ的取向有关
	float CosTheTa=arm_cos_f32(Theta);
	float SinTheTa=arm_sin_f32(Theta);
	...
	else//未识别到目标
	{
			...
			if(chassis.Radar_totalX > (init_x+MAX_DISTANCE-step_x)) PC_setx = init_x - MAX_DISTANCE;
			if(chassis.Radar_totalX < (init_x-MAX_DISTANCE+step_x)) PC_setx = init_x + MAX_DISTANCE;
