...
void SD_DeInit(void)
{
  ...
  GPIO_PinAFConfig(GPIOB,GPIO_PinSource3,GPIO_AF5_SPI3);
  GPIO_PinAFConfig(GPIOB,GPIO_PinSource4,GPIO_AF5_SPI3);
...
static void GPIO_Configuration(void)
{
  ...
  RCC_AHB1PeriphClockCmd(SD_CS_GPIO_CLK | SD_SPI_MOSI_GPIO_CLK | 
...
//  GPIO_Init(SD_DETECT_GPIO_PORT, &GPIO_InitStructure);
  ...
  SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;  //这里这个引脚没有用到，只要我手动把PB6拉低就能使能
  SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_64;//SPI_BaudRatePrescaler_2;
  SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;
  SPI_InitStructure.SPI_CRCPolynomial = 7;
  SPI_Init(SD_SPI, &SPI_InitStructure);
  
//  SPI_I2S_ClearFlag(SD_SPI,SPI_I2S_IT_RXNE);
//  SPI_I2S_ITConfig(SD_SPI,SPI_I2S_IT_RXNE,ENABLE);
...
uint8_t SD_SendCmd(uint8_t Cmd, uint32_t Arg, uint8_t Crc)
{
  ...
  for (i = 0; i < 6; i++)
  {
    ret = SD_WriteByte(Frame[i]); /*!< Send the Cmd bytes */
  }
  return ret;
}
