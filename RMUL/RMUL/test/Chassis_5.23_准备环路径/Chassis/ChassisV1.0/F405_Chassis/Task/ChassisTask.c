	...
	if(1)
	{
		switch(chassis.PC_State)
		{
			...
			case TOPATH1://路径1去
				...
				chassis.NavigatePathNum = 3;//去前哨战
				NAV_PATH_Act(RADAR);
...
void Patrol_Act()
{
	...
	else//未识别到目标
	{
			...
			if(chassis.Radar_totalX > ((init_x-Patrol_Cnt*Patrol_X)+MAX_DISTANCE-step_x)) PC_setx = (init_x-Patrol_Cnt*Patrol_X) - MAX_DISTANCE;
			if(chassis.Radar_totalX < ((init_x-Patrol_Cnt*Patrol_X)-MAX_DISTANCE+step_x)) PC_setx = (init_x-Patrol_Cnt*Patrol_X) + MAX_DISTANCE;
